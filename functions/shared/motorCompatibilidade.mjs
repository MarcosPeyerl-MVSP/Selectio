import { normalizarRubricaDaVaga, textoParaLista } from './rubricaCompatibilidade.mjs'

export const ANALISE_VERSAO = '2.1.0'
...
const criarCriterio = ({ id, categoria, titulo, peso, resultado, evidencias = [], detalhes = [], fator, cobertura = resultado === 'nao_comprovado' || resultado === 'nao_aplicavel' ? 0 : 1, aplicavel = true }) => {
  ...
  return {
    ...
    pontosPrecisos: pesoAplicavel * fatorFinal,
...
export const avaliarCompatibilidade = ({ candidato, vaga, textoCurriculo = '', semantica = {}, extracao = {} }) => {
  ...
  const pontosPrecisos = criteriosAplicaveis.reduce((total, criterio) => total + criterio.pontosPrecisos, 0)
  ...
  return {
    ...
    notaPrecisa: pesoTotal ? Number(((pontosPrecisos / pesoTotal) * 100).toFixed(12)) : 0,
